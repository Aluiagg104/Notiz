...
struct ContentView: View {
    ...
    struct EinkaufsListenView: View {
        ...
        @Binding var einkaufslistenMenge: [Int] // Neue Binding-Variable für die Menge
        var notizenTitle: String
        var saveData: () -> Void
        @Binding var EinkaufslistenPickerSelection: String
        @Binding var einkaufslistenNotizInhalt: String
        @Binding var EinkaufslistenItemQuantity: Int // Binding für die Menge des neuen Produkts
        ...
        var body: some View {
            VStack {
                ...
                if EinkaufslistenPickerSelection == "Liste" {
                    VStack {
                        List {
                            // Zeige den Produktnamen und die Menge zusammen
                                ...
                                Form {
                                    ...
                                    Section {
                                        Button(action: {
                                            ...
                                            isSheetOpen.toggle() // Popup schließen nach dem Hinzufügen
                                        }) {
                                            Text("Hinzufügen")
                                        }
                                    }
        ...
        private func addEinkaufslistenItem() {
            if !einkaufslistenNewElement.isEmpty {
                ...
                einkaufslistenMenge.append(EinkaufslistenItemQuantity) // Menge speichern
                einkaufslistenNewElement = ""
                EinkaufslistenItemQuantity = 1 // Menge zurücksetzen
                saveData()
            }
        }
        ...
        private func deleteEinkaufslistenItems(at offsets: IndexSet) {
            ...
            einkaufslistenMenge.remove(atOffsets: offsets) // Entferne auch die Menge
            saveData()
        }
    }
                ...
                List {
                    ForEach(einkaufslistenTitles.indices, id: \.self) { index in
                        NavigationLink(destination: EinkaufsListenView(
                            ...
                            notizenTitle: einkaufslistenTitles[index],
                            saveData: saveData,
                ...
                if pickerSelected == "Notiz" {
                    List {
                        ForEach(notizenTitles.indices, id: \.self) { index in
                            NavigationLink(destination: NotizInhaltView(notizenInhalt: $notizenInhalt[index], notizenTitle: notizenTitles[index], saveData: saveData)) {
                                VStack(alignment: .leading) {
