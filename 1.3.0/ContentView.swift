...
struct ContentView: View {
    ...
    @State private var showPopup = false // Steuert die Anzeige des Popups
            ...
            VStack {
                ...
                if isEditing {
                    TextField("Einkaufslistenname bearbeiten", text: $notizenTitle)
                        ...
                        .onChange(of: notizenTitle) { oldValue, newValue in
    ...
    struct EinkaufslistenListView: View {
        ...
        var body: some View {
            VStack {
                if einkaufslistenTitles.isEmpty {
                    VStack {
                        Text("Keine Einkaufslisten vorhanden")
                            .foregroundColor(.gray)
                            .padding()
                        Text("Klicke auf das + symbol um eine \n neue Einkaufsliste zu erstellen")
                            .foregroundColor(.gray)
                    }
                }
    ...
    struct SettingsView: View {
        ...
        @Environment(\.colorScheme) var colorScheme
        var saveData: () -> Void  // Hier fügt man `saveData` als Parameter hinzu

        var body: some View {
            List {
                Section(header: Text("Erscheinungsbild").font(.headline).foregroundColor(.primary)) {
                    HStack {
                        Text("Theme:")
            ...
            .onAppear(perform: loadThemeSetting)
        }
        ...
        private func applyTheme(isDark: Bool) {
            ...
            isDarkMode = isDark
            saveData() // Speichert die Änderung sofort
        }

        private func loadThemeSetting() {
            let defaults = UserDefaults.standard
            if let savedDarkMode = defaults.value(forKey: "isDarkMode") as? Bool {
                isDarkMode = savedDarkMode
                applyTheme(isDark: isDarkMode)
            } else {
                ...
                VStack {
                    ...
                    HStack {
                        ...
                        .sheet(isPresented: $showPopup) {
                            SettingsView(isDarkMode: $isDarkMode, saveData: saveData) // Sicherstellen, dass saveData korrekt übergeben wird
                        }
    ...
    private func loadData() {
        ...
        // Lade den Dark Mode-Wert
        let defaults = UserDefaults.standard
        if let savedDarkMode = defaults.value(forKey: "isDarkMode") as? Bool {
            isDarkMode = savedDarkMode
            applyTheme(isDark: isDarkMode) // Theme direkt anwenden
        } else {
            isDarkMode = colorScheme == .dark
            applyTheme(isDark: isDarkMode) // Standard-Theme basierend auf System anwenden
        }
    }
