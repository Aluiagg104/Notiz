...
struct ContentView: View {
    ...
    struct NotizInhaltView: View {
        ...
        @Binding var notizenTitle: String // Hier wird der Titel als Binding hinzugefügt
        @State private var isEditing = false // Zustand für den Bearbeitungsmodus
        var saveData: () -> Void
            ...
            VStack {
                if isEditing {
                    ...
                    TextField("Notiz Titel", text: $notizenTitle)
                        .font(.headline)
                        .padding()
                        .onChange(of: notizenTitle) { _ in
                            saveData() // Speichere die Änderungen automatisch
                        }
                } else {
    ...
    struct EinkaufsListenView: View {
        ...
        var body: some View {
            VStack {
                ...
                if isEditing {
                    TextField("Einkaufslistenname bearbeiten", text: $notizenTitle, onCommit: {
                        saveData() // Speichern, wenn der Bearbeitungsmodus beendet wird
                    })
                    .font(.headline)
                    .padding()
                }
                    ...
                    List {
                        ForEach(notizenTitles.indices, id: \.self) { index in
                            NavigationLink(destination: NotizInhaltView(
                                ...
                                notizenTitle: $notizenTitles[index], // Hier wird der Titel als Binding übergeben
                                saveData: saveData
                            )) {
                                VStack(alignment: .leading) {
